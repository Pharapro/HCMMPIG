...
{
    public class ClsHREmpSupense : IClsHREmpSupense
    {
        protected IUnitOfWork unitOfWork;
        public SYUser User { get; set; }
        public SYUserBusiness BS { get; set; }
        public string ScreenId { get; set; }
        public HREFLeave Header { get; set; }
        public string DocType { get; set; }
        public string MessageError { get; set; }
        public List<HREFLeave> ListHeader { get; set; }
        public List<ClsRequestSupense> ListRequestPending { get; set; }
        ...
        public ClsHREmpSupense()
        {
            User = SYSession.getSessionUser();
            BS = SYSession.getSessionUserBS();
            OnLoad();
        }
        public void ProcessSupense(string userName, string Type)
        {
            OnLoad();
            ...
            var listReqSupense = unitOfWork.Set<HREFLeave>().AsQueryable()
                .Where(w => w.Status == pending && w.Type == Type)
                .ToList();
            ...
            var ListLeaveCreater = unitOfWork.Set<HREFLeave>().AsQueryable().AsEnumerable()
                    ...
                    .ToList();
            ListHeader = ListLeaveCreater.Where(w => w.Type == Type).ToList();
        }
        private void AddPendingToList(ExDocApproval item, List<HREFLeave> listReqSupense)
        {
            foreach (var read in listReqSupense)
            {
                var ListPen = new ClsRequestSupense();
        ...
        public string CreateSuspen(string Type)
        {
            OnLoad();
            try
            {
        ...
        public string RequestForApprove(string Doc, string fileName, string URL, bool issuspen)
        {
            string Type = "MATERNITY";
            string TypeLeave = "Maternity";
            if (issuspen)
            {
                Type = "SUSPENSE";
                TypeLeave = "Suspense";
            ...
            try
            {
                var objMatch = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == Doc && w.Type == Type);
                ...
                if (StaffApp != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(StaffApp.Email))
                    {
                        try
                        {
                            ...
                            if (EmailConf != null)
                            {
                                ...
                                str += $" <br /> I would like to Request {TypeLeave} <br />from <b>{objMatch.FromDate.ToString("dd/MMM/yyyy")} </b>";
                                ...
                                string subject = $"Request {TypeLeave}";
                    ...
                    {
                        string str = $" I would like to Request {TypeLeave} %0A from<b>{objMatch.FromDate.ToString("dd/MMM/yyyy")} </b>";
                        str += $" to <b> {objMatch.ToDate.ToString("dd/MMM/yyyy")} </b>";
       ...
       .SelectMany(g => g             // Flatten groups while maintaining level order
        ...
        public string Edit(string Doc)
        {
            ...
            try
            {
                var objMatch = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == Doc);
                if (objMatch == null)
                {
                    return "DOC_NE";
                }
        ...
        public string Reject(string Document, bool issuspen)
        {
            string Type = "MATERNITY";
            string TypeLeave = "Maternity";
            if (issuspen)
                ...
                foreach (var Doc in c)
                {
                    ...
                    if (EmpCreate != null && !string.IsNullOrEmpty(EmpCreate.Email) && StaffView != null)
                    {
                        ...
                        if (EmailConf != null)
                        {
                            ...
                            body += $"<br /> I’m reject to the  {TypeLeave} request for  <b> {StaffView.Title} {objmatch.EmpName}</b> ";
                    ...
                    if (excfObject != null && !string.IsNullOrEmpty(excfObject.Telegram))
                    {
                        ...
                        str += $" %0A I’m reject to the  {TypeLeave} request for %0A <b>{StaffView.Title} {objmatch.EmpName}</b>";
        ...
        public string Cancel(string OTRNo, string Type)
        {
            OnLoad();
            try
            {
                ...
                foreach (var ID in c)
                {
                    var objMatch = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == ID && w.Type == Type);
                    if (objMatch == null)
                    {
                        return "INVALIED";
                    }
        ...
        public string approveTheDoc(string Inc, string fileName, string URL, bool issuspen)
        {
            OnLoad();
            ...
            try
            {
                latestCareer = ProcessApproval(Inc, fileName, URL, issuspen);
                unitOfWork.Commit();
            }
            catch (DbEntityValidationException e)
            {
        ...
        private string ProcessApproval(string Inc, string fileName, string URL, bool issuspen)
        {
            string Type = "MATERNITY";
            if (issuspen)
            {
            ...
            var obj = unitOfWork.Set<HREFLeave>()
                .FirstOrDefault(x => x.DocNo == Inc && x.Type == Type);
            ...
            NotifyNextApprover(obj, objapprover, fileName, openStatus, issuspen);
            UpdateProbationStatus(obj, approvalList, openStatus);

            if (obj.Status == SYDocumentStatus.APPROVED.ToString())
            {
                ProcessProbation(obj, fileName, objapprover, issuspen);
            }
        ...
        private void NotifyNextApprover(HREFLeave record, List<ExDocApproval> approvals, string fileName, string openStatus, bool issuspen)
        {
            ...
            SendApprovalEmail(record, staffProfile, fileName, issuspen);
        }
        private void SendApprovalEmail(HREFLeave record, HRStaffProfile approver, string fileName, bool issuspen)
        {
            string TypeLeave = "Maternity";
            if (issuspen)
            {
            ...
            <br/><br/>I would like to Request {TypeLeave} 
        ...
        private void UpdateProbationStatus(HREFLeave record, List<ExDocApproval> approvals, string openStatus)
        {
        ...
        private void ProcessProbation(HREFLeave record, string fileName, List<ExDocApproval> approvals, bool issuspen)
        {
            ...
            UpdateLeave(record, staff);
            NotifyRequestor(record, fileName, approvals, issuspen);
        }
        private void UpdateLeave(HREFLeave record, HRStaffProfile staff)
        {
            string Units = "Day";
            int Increment = GenerateLeaveObject.GetLastIncrement();
            var _param = unitOfWork.Set<PRParameter>().AsQueryable().FirstOrDefault(w => w.Code == staff.PAEMAIL);
            decimal WHOUR = 8;
            if (_param != null) WHOUR = (decimal)_param.WHOUR;
            var HeaderEmpLeave = new HREmpLeave();
            HeaderEmpLeave.EmpCode = staff.EmpCode;
            HeaderEmpLeave.FromDate = record.FromDate;
            HeaderEmpLeave.ToDate = record.ToDate;
            HeaderEmpLeave.LeaveCode = "SUP";
            ...
            HeaderEmpLeave.Reason = record.Description;
            HeaderEmpLeave.CreateBy = User.UserName;
            HeaderEmpLeave.CreateOn = DateTime.Now;
            int Line = 0;
            decimal NoDay = 0;
            for (DateTime date = record.FromDate; date <= record.ToDate; date = date.AddDays(1))
            {
                Line += 1;
            ...
            unitOfWork.Add(HeaderEmpLeave);
        }
        private void NotifyRequestor(HREFLeave record, string fileName, List<ExDocApproval> approvals, bool issuspen)
        {
            string Type = "MATERNITY";
            string TypeLeave = "Maternity";
            if (issuspen)
            ...
            if (EmpCreate != null && !string.IsNullOrEmpty(EmpCreate.Email))
            {
                ...
                <br/><br/>I’m approved to the {TypeLeave} Request for:
            ...
            %0AI’m approved to the {TypeLeave} Request for: {staffView.AllName}
        ...
        public class ClsRequestSupense
        {
