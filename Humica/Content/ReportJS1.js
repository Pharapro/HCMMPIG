...
//-------------------Businessunit Code------------------------------------
function OnSelectionChangedBU(s, e) {
    s.GetSelectedFieldValues("Code", GetSelectedFieldValuesCallbackBU);
}
function GetSelectedFieldValuesCallbackBU(values) {
    _selectedIDsBU = "";

    for (var index = 0; index < values.length; index++) {
        _selectedIDsBU+= values[index] + textSeparator;

    }
    if (_selectedIDsBU.length > 0) {
        _selectedIDsBU = _selectedIDsBU.substring(0, _selectedIDsBU.length - 1);
    }

    checkComboBoxBU.SetText(_selectedIDsBU);
}
function clearSelectedBU(s, e) {
    checkComboBoxBU.SetText("");
    BUGridListSelector.UnselectAllRowsOnPage();
    _selectedIDsBU = "";
