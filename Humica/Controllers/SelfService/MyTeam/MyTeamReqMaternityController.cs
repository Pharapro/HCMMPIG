...
{
    public class MyTeamReqMaternityController : Humica.EF.Controllers.MasterSaleController
    {
        private const string SCREEN_ID = "ESS0000029";
        private const string URL_SCREEN = "/SelfService/MyTeam/MyTeamReqMaternity/";
        private string Index_Sess_Obj = SYSConstant.BUSINESS_OBJECT_MODEL + SCREEN_ID;
        private string ActionName;
        private string KeyName = "DocNo";
        private string PATH_FILE = "12313123123sadfsdfsdfsdf";
        private string DOCTYPE = "REMATER";
        private string _LOCATION_ = "_LOCATION_";
        IClsHREmpSupense BSM;
        IUnitOfWork unitOfWork;
        public MyTeamReqMaternityController()
            : base()
        {
        ...
        public ActionResult Index()
        {
            ...
            BSM.ProcessSupense(userName, "MATERNITY");
        ...
        public ActionResult Create(ClsHREmpSupense collection)
        {
            ...
            if (!string.IsNullOrEmpty(collection.Header.EmpCode))
            {
                ...
                BSM.DocType = DOCTYPE;
                string msg = BSM.CreateSuspen("MATERNITY");
        ...
        public ActionResult Details(string Doc)
        {
            ...
            if (Doc != null)
            {
                ...
                BSM.Header = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == Doc && w.Type == "MATERNITY");
                if (BSM.Header != null)
                {
        ...
        public ActionResult DocumentViewerPartial(string Doc)
        {
            ...
            UserConf(ActionBehavior.VIEWONLY);
            ActionName = "Print";
            var SD = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == Doc && w.Type == "MATERNITY");
            if (SD != null)
            {
                try
                {
                    ViewData[Humica.EF.SYSConstant.PARAM_ID] = Doc;
                    var sa = new FRMMaternity();
        ...
        public ActionResult DocumentViewerExportTo(string Doc)
        {
            ...
            if (SD != null)
            {
                FRMMaternity reportModel = new FRMMaternity();

                reportModel = (FRMMaternity)Session[Index_Sess_Obj + ActionName];
        ...
        public ActionResult RequestForApproval(string DocNo)
        {
            ...
            {
                BSM.ScreenId = SCREEN_ID;
                var Objmatch = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == DocNo && w.Type == "MATERNITY");
                ...
                FRMMaternity sa = new FRMMaternity();
                var reportHelper = new clsReportHelper();
                string path = reportHelper.Get_Path(SCREEN_ID);
                if (!string.IsNullOrEmpty(path))
                {
                ...
                string msg = BSM.RequestForApprove(DocNo, fileName, URL, false);
                if (msg == SYConstant.OK)
                {
        ...
        public ActionResult Cancel(string DocNo)
        {
            ...
            {
                BSM.ScreenId = SCREEN_ID;
                string msg = BSM.Cancel(DocNo, "MATERNITY");
                if (msg == SYConstant.OK)
                {
        ...
        public ActionResult Approve(string Doc)
        {
            ...
            foreach (var Document in c)
            {
                ...
                var Objmatch = unitOfWork.Set<HREFLeave>().AsQueryable().FirstOrDefault(w => w.DocNo == Document && w.Type == "MATERNITY");
