...
namespace Humica.Logic.CF
{
    ...
    public partial class CFNumberRank
    {
        ...
        public CFNumberRank(string Comapany, string Branch, string Bu, bool IsRunNext = false)
        {
            var DocConf = new List<HREmpCode>();
            DocConf = DBX.HREmpCodes.ToList();
            if (!string.IsNullOrEmpty(Comapany))
                DocConf = DocConf.Where(w => w.Company == Comapany).ToList();
            if (!string.IsNullOrEmpty(Branch))
                DocConf = DocConf.Where(w => w.Branch == Branch).ToList();
            if (!string.IsNullOrEmpty(Bu))
                DocConf = DocConf.Where(w => w.BU == Bu).ToList();
            if (DocConf.Any())
            {
                Number = DB.BSNumberRanks.Find(DocConf.FirstOrDefault().NumberRank);
                NumberItem = DB.BSNumberRankItemNs.Find(DocConf.FirstOrDefault().NumberRank, DocConf.FirstOrDefault().NumberRankItem);
        ...
        public CFNumberRank(string DocType, DocConfType Doc, int Period, bool IsRunNext = false, bool IsNextAssetCode = false)
        {
            try
            {
                if (Doc == DocConfType.Normal)
				...
				if (Doc == DocConfType.FixedAsset)
				{
					...
					if (Number.IsFlagYear == false && Number.IsHasIndentification == false)
					{
						NumberItem = DB.BSNumberRankItemNs.Find(DocType, Number.Length.ToString());
					}
					else if (Number.IsFlagYear == false && Number.IsHasIndentification == true)
					{
						NumberItemHasIdentity = DB.BSNumberRankNies.Find(DocType, Number.Length.ToString(), SYConstant.DEFAULT_PLANT);
					}
					else if (Number.IsFlagYear == true && Number.IsHasIndentification == true)
					{
						NumberItemHasIdentityYear = DB.BSNumberRankItems.Find(DocType, Number.Length.ToString(), SYConstant.DEFAULT_PLANT, Period);
					}
				}
				if (IsRunNext == true)
                {
